...
export default function DeckPage() {
  ...
  // Use ?showDrafts=1 to include them. PDF export is just `?print=1`, so
  // drafts stay out of `slides.pdf` unless explicitly opted in.
  const visibleSlides = useMemo(
    () => (showDrafts ? slides : slides.filter((s) => !s.draft)),
    [showDrafts],
  );
  ...
  const overviewHref = useMemo(() => {
    ...
    if (showDrafts) params.set("showDrafts", "1");
      ...
      <div className="min-h-screen bg-slate-100 p-8">
        <div className="mx-auto max-w-6xl">
          <header className="mb-6 flex items-end justify-between">
            <div>
              <h1 className="text-2xl font-bold text-slate-900">Slides</h1>
  ...
  if (visibleSlides.length === 0) {
    return (
      <div className="flex h-screen w-screen items-center justify-center bg-slate-100 text-slate-600">
        No slides. Add one in <code className="mx-1">lib/slides.tsx</code>.
      </div>
    );
  }
